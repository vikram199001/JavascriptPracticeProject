...
if (location.pathname == "/html%20starter%20pages/senate-attendance-starter-page.html") {

    url = "https://api.propublica.org/congress/v1/113/senate/members.json";
    getData(url);
}
if (location.pathname == "/html%20starter%20pages/house-attendance-starter-page.html") {
...
function leastMissedVotes() {
    let abc = [...allMemberVotes.sort((a, b) => parseFloat(a.missed_votes) - parseFloat(b.missed_votes))];
    console.log(abc);

    let leastmissedV = abc[Math.round(abc.length * 0.1)].missed_votes;
    console.log(leastmissedV);
    // results.push(leastmissedV)
    let results = []

    for (k = 0; k < abc.length; k++) {
        if (abc[k].missed_votes <= leastmissedV) {
            results.push(abc[k]);
        } else {
            break;
        }
...
function mostMissedVotes() {
    let cba = [...allMemberVotes.sort((a, b) => parseFloat(b.missed_votes) - parseFloat(a.missed_votes))];
    console.log(cba);

    let mostmissedV = cba[Math.round(cba.length * 0.1)].missed_votes;
    console.log(mostmissedV);
    let resultSS = []

    for (k = 0; k < cba.length; k++) {
        if (cba[k].missed_votes >= mostmissedV) {
            resultSS.push(cba[k]);
        } else {
            break;
        }
...
function generateTbl(id, perc) {
    ...
    var text1 = document.createTextNode("No.of Missed Votes");
    cell2.appendChild(text1);
    rowq.appendChild(cell2);
    var cell3 = document.createElement("td");
    var text2 = document.createTextNode("% of Missed Votes");
    ...
    for (i = 0; i < perc.length; i++) {
        ...
        row.insertCell().innerHTML = perc[i].missed_votes;
        row.insertCell().innerHTML = perc[i].missed_votes_pct;
